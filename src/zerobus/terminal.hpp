...
class Terminal: public IListener {
public:
     ...
     */
    bool send_message( ChannelID channel, MessageContent msg, ConversationID cid = 0, MsgFlags flags = MsgFlags::priorityNormal) {
        return _bus.send_message(this, channel, msg, cid, flags);
    }
...
class CallbackTerminal : public Terminal {
public:
    ...
    virtual void on_add_to_group(ChannelID group_name, ChannelID, ConversationID cid) noexcept override{
        if constexpr(std::invocable<Callback, Terminal &, const AddedToGroup &>) {
...
template<TerminalCallback Callback>
auto Bus::new_terminal(Callback &&cb) {
    return CallbackTerminal(*this, std::forward<Callback>(cb));
}
template<TerminalCallback Callback>
std::unique_ptr<Terminal> Bus::new_terminal_unique(Callback &&cb) {
    return std::unique_ptr<CallbackTerminal<Callback> >(*this, std::forward<Callback>(cb));
}
template<TerminalCallback Callback>
std::shared_ptr<Terminal> Bus::new_terminal_shared(Callback &&cb) {
    return std::shared_ptr<CallbackTerminal<Callback> >(*this, std::forward<Callback>(cb));
