...
namespace BinMsgType{
    using type = char;
    constexpr type channel_reset = 0;
    constexpr type set_channels=1;
    constexpr type add_channels=2;
    constexpr type erase_channels=3;
    constexpr type message=4;
    constexpr type update_serial=5;
    constexpr type new_session=6;
    constexpr type add_group=7;
    constexpr type close_group=8;
    constexpr type empty_group=9;
    constexpr type no_route=10;
};
...
public:
    ...
    void parse(std::string_view message) {
        ...
        BinMsgType::type t = *from;
        ++from;
        switch (t) {
            default: break;
            case MsgAddChannels::uid:parse_msg<MsgAddChannels>(from, to); break;
            case MsgEraseChannels::uid:parse_msg<MsgEraseChannels>(from, to); break;
            case MsgSetChannels::uid:parse_msg<MsgSetChannels>(from, to); break;
            case MsgChannelReset::uid:parse_msg<MsgChannelReset>(from, to); break;
            case MsgMessage::uid:parse_msg<MsgMessage>(from, to); break;
            case MsgUpdateSerial::uid:parse_msg<MsgUpdateSerial>(from, to); break;
            case MsgNewSession::uid:parse_msg<MsgNewSession>(from, to); break;
            case MsgAddToGroup::uid:parse_msg<MsgAddToGroup>(from, to); break;
            case MsgCloseGroup::uid:parse_msg<MsgCloseGroup>(from, to); break;
            case MsgGroupEmpty::uid:parse_msg<MsgGroupEmpty>(from, to); break;
            case MsgNoRoute::uid:parse_msg<MsgNoRoute>(from, to); break;
        }
    }
...
protected:
    ...
    static std::size_t get_msg_size(const MsgMessage &msg) {
    ...
    static void serialize_msg(const MsgMessage &msg, char *iter) {
    ...
    void parse_msg(const char *from, const char *to) {
        if constexpr(std::is_base_of_v<MsgChannelsBase, Msg>) {
            ...
            if constexpr(std::is_same_v<Msg, MsgAddToGroup>) {
            ...
            } else if constexpr(std::is_same_v<Msg, MsgMessage>) {
                bin::decode_number(msg.cid, from, to);
                bin::decode_string(msg.sender, from, to);
                ...
                static_assert(std::is_same_v<Msg, MsgChannelReset>);
            }
        }
