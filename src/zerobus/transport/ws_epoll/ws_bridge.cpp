...
:Bridge(owner._bus, owner.create_transport(this, _bridge_parser))
,_owner(owner)
,_ws_parser(_input_buffer, false) {}
...
void WsBridge::Peer::send_message(const ws::Message &msg) {
    ...
    std::size_t idx = sz;
    ws::build(msg, [&](char c){_output_buffer[idx++]=c;}, masking_ptr);
    send_buffer({_output_buffer.data() + sz,idx - sz});
}

bool WsBridge::Peer::conn_error() {
    if (_reconnect_addr.empty()) return false;
...
bool WsBridge::Peer::send_buffer(std::string_view data) {
    ...
    while (!data.empty()) {
        pfd.revents = 0;
        if (poll(&pfd, 1, write_timeout) == 0) return false;
...
std::optional<std::string_view> WsBridge::Peer::read_http_header(std::string_view data) {
    ...
    if (_mode == PeerOpMode::await_response) {
        ...
        if (!upgrade || !conn_upgrade || sock_key != _ws_accept) return std::nullopt;
        _ws_accept.clear();
            ...
            try {
                if (_owner._config.document_root) {
                    handle_http_request(data, [&](std::string_view txt){
                        if (!send_buffer(txt)) throw false;
                    }, *_owner._config.document_root);
                }
            } catch (...) {}
...
void WsBridge::ensure_threads_running() {
    ...
    if (r) _pool.add_threads(r, [this](std::stop_token stp){
        worker(stp);
    });
}
...
void WsBridge::worker(std::stop_token stp) {
    while (!stp.stop_requested()) {
        auto wt = _epoll.wait();
        if (!wt || wt->ident == 0) continue;
        PHandleData hdata;
        {
            std::unique_lock lk(_mx);
            auto iter = _handles.find(wt->ident);
            if (iter == _handles.end()) continue;
            hdata = iter->_value;
        }
